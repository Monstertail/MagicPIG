...
parser.add_argument("--QR", type=float, default=0.05)
...
def main():
    ...
    if args.Q == 0 or args.Q == 3:
        import torch
        llm.sparse = torch.tensor(llm.sparse).sum(0)
        decode_tokens = sum(llm.decode_tokens)
        llm.sparse = llm.sparse / decode_tokens
        print("Avg Sparsity: {:.3f}".format(llm.sparse))
