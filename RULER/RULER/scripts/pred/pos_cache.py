...
class PosCache(Cache):
    ...
    def __init__(self, K, L, mode = "anns", window = 64, resample: bool = False, sample_layer: int = 1, resample_layer: int = 12, ) -> None:
    ...
    def update(
        ...
        Parameters:
            key_states (`torch.Tensor`):
                The new key states to cache. (batch, num_heads, seq_len, head_dim)
            value_states (`torch.Tensor`):
    ...
    @classmethod
    def from_legacy_cache(cls, past_key_values: Optional[Tuple[Tuple[torch.FloatTensor]]] = None) -> "TopkCache":
